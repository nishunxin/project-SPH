            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                ...
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{'icon-up':isAsc,'icon-dowm':isDesc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        categoryName: "",  //分类的名字
        keyword: "",  //关键字
        order: "1:desc",  //排序:默认是综合：降序
        pageNo: 1,  //分页器代表第几页
        pageSize: 10,  //每一页所展示的数据个数
        props: [],  //平台售卖属性操作所带的数据
        trademark: "",    //品牌
      },
    };
  ...
  methods: {
    //向服务器发请求获取search模块数据，根据返回不同数据进行展示并封装成一个函数，每次使用只需调用即可
    getData() {
    ...
    removeCategoryName() {
      ...
      //带给服务器参数说明可有可无的：为空会带给服务器，为undefined不会带给服务器
    ...
    //排序的操作
    changeOrder(flag) {
      //flag形参：它是一个标记，代表用户点击的是综合1还是价格2
  ...
  watch: {
    ...
    $route(newValue,oldValue) {
      ...
      //再次发起Ajax请求
      this.getData();
      //每一次请求完毕，应清空相应的1 2 3 级分类的id，让他接受下一次请求相应的1 2 3级的id
